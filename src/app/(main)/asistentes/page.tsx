  ...
  return (
    <div className="py-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-100">Asistentes</h2>
          <p className="text-sm text-slate-400">{attendees.length} personas</p>
        </div>
        <button
          onClick={() => setShowForm(!showForm)}
          className="flex items-center gap-1.5 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 active:bg-emerald-700 text-white font-medium rounded-xl transition-colors text-sm"
        >
        ...
        <form onSubmit={handleAdd} className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-4">
          <div>
            ...
            <input
              ...
              required
              autoFocus
              className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-base"
            />
          </div>
          <label className="flex items-center gap-3 cursor-pointer">
            <div
              onClick={() => setIsVeg(!isVeg)}
              className={`w-11 h-6 rounded-full transition-colors relative ${isVeg ? "bg-emerald-600" : "bg-slate-700"}`}
            >
            ...
            <span className="text-slate-300 text-sm">Vegetariano/a</span>
          </label>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {days.map((day) => (
              <div key={day} className="flex flex-col items-center min-w-[3.5rem] px-2 py-2 bg-slate-800 rounded-lg">
                <span className="text-xs text-slate-400">{formatDateShort(day)}</span>
                <span className="text-lg font-bold text-slate-100">{presentCount(day)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="space-y-3">
        {attendees.map((att) => (
          <div key={att.id} className="bg-slate-900 border border-slate-800 rounded-xl p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center text-slate-300 font-bold text-sm uppercase">
                  {att.name.slice(0, 2)}
                </div>
                <div>
                  <p className="text-slate-100 font-medium">{att.name}</p>
                  <div className="flex items-center gap-2">
                    {att.is_vegetarian && (
                      <span className="text-xs text-emerald-400">Vegetariano/a</span>
                    )}
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-1">
                <button
                  ...
                  className={`p-2 rounded-lg transition-colors ${att.is_vegetarian ? "text-emerald-500 bg-emerald-600/10" : "text-slate-500 hover:text-slate-300"}`}
