...
export default function HistorialPage() {
  ...
  return (
    <div className="py-6 space-y-4">
      <h2 className="text-xl font-bold text-tierra-100">Historial</h2>
      ...
      {events.length === 0 && history.length === 0 ? (
        <div className="text-center py-8">
          ...
          <h3 className="text-lg font-bold text-tierra-100 mb-2">Sin historial</h3>
        ...
        <div className="space-y-3">
          {events.map((evt) => {
            ...
            return (
              <div key={evt.id} className="bg-tierra-900 border border-tierra-800 rounded-xl p-4">
                <h3 className="text-tierra-100 font-semibold mb-1">{evt.name}</h3>
                <p className="text-xs text-tierra-400 mb-2">{evt.start_date} — {evt.end_date}</p>
                {snap && (
                  <div className="grid grid-cols-2 gap-2 mt-2">
                    <div className="bg-tierra-800 rounded-lg p-2 text-center">
                      <p className="text-sm font-bold text-tierra-100">${snap.total_expenses?.toLocaleString("es-AR") || "0"}</p>
                      <p className="text-xs text-tierra-400">Total gastado</p>
                    </div>
                    <div className="bg-tierra-800 rounded-lg p-2 text-center">
