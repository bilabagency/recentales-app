...
export default function CamasPage() {
  ...
  return (
    <div className="py-6 space-y-4">
      <h2 className="text-xl font-bold text-slate-100">Camas</h2>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
        <div className="grid grid-cols-2 gap-4">
          <div className="text-center">
            <p className="text-2xl font-bold text-slate-100">{totalCapacity}</p>
            <p className="text-xs text-slate-400">Lugares disponibles</p>
          </div>
          <div className="text-center">
            <p className={`text-2xl font-bold ${needTents ? "text-amber-400" : "text-emerald-400"}`}>{totalPeople}</p>
      ...
      <div className="space-y-2">
        {beds.map((b) => (
          <div key={b.id} className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${b.bed_type === "double" ? "bg-amber-500/20" : "bg-emerald-600/20"}`}>
                <span className={`text-lg font-bold ${b.bed_type === "double" ? "text-amber-400" : "text-emerald-400"}`}>
                  {b.bed_type === "double" ? "D" : "S"}
                </span>
              </div>
              <div>
                <p className="text-slate-100 font-medium text-sm">{b.label || (b.bed_type === "double" ? "Cama doble" : "Cama simple")}</p>
      ...
      {beds.length === 0 && (
        <div className="text-center py-8">
          <p className="text-slate-400 text-sm">No hay camas cargadas. Se crean automáticamente al crear un evento.</p>
        </div>
      )}
    </div>
  );
