  ...
  return (
    <div className="py-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-100">Bebidas</h2>
          <p className="text-sm text-slate-400">Estimación por persona</p>
        </div>
        <button
      ...
      {showForm && (
        <form onSubmit={handleAdd} className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-2">Quién trae</label>
            <select
              value={selectedAttendee}
              onChange={(e) => setSelectedAttendee(e.target.value)}
              required
              className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-base"
            >
          ...
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-2">Bebida</label>
            <input
              type="text"
              value={drinkName}
              ...
              required
              className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-base"
            />
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button type="submit" disabled={saving} className="flex-1 py-3 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white font-semibold rounded-xl transition-colors">{saving ? "Guardando..." : "Guardar"}</button>
          </div>
        </form>
      )}
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          ...
          <div className="space-y-2">
            {Object.entries(consolidated).map(([key, val]) => (
              <div key={key} className="flex justify-between py-1.5 border-b border-slate-800 last:border-0">
                <span className="text-slate-300 text-sm capitalize">{key.split("_")[0]}</span>
                <span className="text-slate-100 font-medium text-sm">{val.quantity} {val.unit}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {drinks.length > 0 && (
        <div className="space-y-2">
          {drinks.map((d) => {
            ...
            return (
              <div key={d.id} className="bg-slate-900 border border-slate-800 rounded-xl p-3 flex items-center justify-between">
                <div>
                  <p className="text-slate-100 text-sm font-medium capitalize">{d.drink_name}</p>
                  <p className="text-slate-400 text-xs">{att?.name} — {d.quantity} {d.unit}</p>
                </div>
                <button onClick={() => removeDrink(d.id)} className="p-2 text-slate-500 hover:text-red-400 transition-colors">
      ...
      {drinks.length === 0 && !showForm && (
        <div className="text-center py-8">
          <p className="text-slate-400 text-sm">Todavía no hay bebidas cargadas.</p>
        </div>
      )}
    </div>
  );
